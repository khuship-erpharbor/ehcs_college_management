...
class Sportsform(models.Model):
    ...
    sports_ids = fields.One2many(
        comodel_name = "college.sports",
        inverse_name = "sportsform_id",
        string = "Sports"
                )

    def action_sports(self):
        ctx = self.env.context
        sports = self.env['college.sports'].search([('name','=','kabbadi'),('fees','=',23000)])
        print("\n\n\n\n\n",sports)
        res = self.env['college.sports'].create({
            'date':sports.date,
            'fees':sports.fees,
            'sportsform_id':self.id
            })
        return res
